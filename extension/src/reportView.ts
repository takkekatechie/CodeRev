...
import { ComparisonResult, ScanResult, Issue, IssueStatus } from './types';
...
export class ReportView {
  ...
  private serverClient: ServerClient | undefined;
  private scanHistory: ScanResult[] = [];
  private repositoryPath: string | undefined;

  constructor(extensionUri: vscode.Uri, serverClient?: ServerClient) {
   ...
   */
  async show(result: ComparisonResult): Promise<void> {
  ...
  private getHtmlContent(result: ComparisonResult): string {
...
<body>
  ...
  private renderIssuesBySeverity(issues: Issue[], severity: string): string {
  ...
  private renderIssueSection(title: string, issues: Issue[]): string {
    return '';
  }
  ...
  private async handleExport(): Promise<void> {
    ...
    try {
      ...
      const content = await this.serverClient.exportReport(this.currentScanId, format as any);
  ...
  private async handleLoadScan(scanId: string): Promise<void> {
    if (!this.serverClient) {
      vscode.window.showErrorMessage('Server client not available.');
      return;
    }
